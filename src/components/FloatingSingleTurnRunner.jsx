          ...
          <div className="multi-turn-float-compose">
            <div className="single-turn-runner-options">
              <label className="multi-turn-float-session">
                <span>모델 선택</span>
                <select value={selectedModelId || ''} onChange={event => onModelChange(event.target.value)} disabled={isRunning}>
                  {models.map(model => <option key={model.id} value={model.id}>{model.name}</option>)}
                </select>
              </label>
              {selectedModel?.provider === 'Google' && <label className="multi-turn-float-session-count">
                <span>Thinking level</span>
                <select value={selectedThinkingLevel} onChange={event => onThinkingLevelChange(event.target.value)} disabled={isRunning}>
